...
const double MAX_TORQUE_DEFAULT = 4.0; // Nm permitidos en operación
...
void PlanificarMovimiento()
{
    ...
    else if (estado_actual == RobotState::ACTIVE)
    {
        ...
        for (int p = 0; p < 4; ++p)
        {
            ...
            patas[p].coxa.ff_torque  = std::clamp(m_cmd.torques_Nm(0), -max_control, max_control);
            patas[p].femur.ff_torque = std::clamp(m_cmd.torques_Nm(1), -max_control, max_control);
            patas[p].tibia.ff_torque = std::clamp(m_cmd.torques_Nm(2), -max_control, max_control);
