...
void optimizar_recursos_pi4() {
    // 1. AFINIDAD: Forzar la ejecución exclusivamente en el Núcleo 3 (previamente aislado)
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(3, &cpuset); 
    
    if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset) != 0) {
        std::cerr << "[WARN] No se pudo asignar el núcleo 3. ¿Está aislado en cmdline.txt?" << std::endl;
    } else {
        std::cout << "✅ Proceso de control anclado al NÚCLEO 3." << std::endl;
    }

    // 2. PRIORIDAD: Establecer planificador de Tiempo Real (FIFO)
    // Esto evita que otros procesos le quiten tiempo de CPU al control
    struct sched_param param;
    param.sched_priority = 80; // Prioridad alta (1-99)
    
    if (sched_setscheduler(0, SCHED_FIFO, &param) == -1) {
        std::cerr << "[ERROR] Falló el modo Real-Time. ¡DEBES EJECUTAR CON SUDO!" << std::endl;
    } else {
        std::cout << "✅ Prioridad de Tiempo Real activada (SCHED_FIFO)." << std::endl;
    }

    // 3. BLOQUEO DE MEMORIA: Evita que el kernel mueva el programa al disco (swap)
    if (mlockall(MCL_CURRENT | MCL_FUTURE) == -1) {
        std::cerr << "[WARN] No se pudo bloquear la memoria en RAM." << std::endl;
    }
}
...
{
    // --- NUEVO: Optimización de recursos ---
